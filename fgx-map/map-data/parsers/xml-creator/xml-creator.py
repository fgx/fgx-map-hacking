...
# geographiclib 1.22 by (c) Charles Karney
from geographiclib.geodesic import Geodesic

if sys.argv[1] == "--help" or sys.argv[1] == "-h" or sys.argv[1] == "":
   print "Usage: python xml-creator.py <file.dat>"
   sys.exit(0)
...
# minidom "to pretty" means very ugly, so thanks to re and stackover-nbolton
uglyXml = apt.toprettyxml()
text_re = re.compile('>\n\s+([^<>\s].*?)\n\s+</', re.DOTALL)    
prettyXml = text_re.sub('>\g<1></', uglyXml)

# writing the outputfile, same location as .dat
outputfile = inputfile.replace(".dat", ".xml")
